...
colonnes_a_garder = ['Numéro Région', 'libellé', 'nom court', 'secteur d\'établissement',
                     'Région', 'Departement2', 'Page Wikipédia en français','géolocalisation']
df_etablissement = df_etablissement[colonnes_a_garder]


# Séparer la colonne en latitude et longitude
df_etablissement[['latitude_ecole', 'longitude_ecole']] = df_etablissement['géolocalisation'].str.split(',', expand=True)

# Nettoyer les espaces éventuels
df_etablissement['latitude_ecole'] = df_etablissement['latitude_ecole'].astype(float)
df_etablissement['longitude_ecole'] = df_etablissement['longitude_ecole'].str.strip().astype(float)
